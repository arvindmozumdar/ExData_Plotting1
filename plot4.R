...
par(mfrow = c(2,2))
## Top left graph
with(hhpc.sub, plot(Global_active_power, x = time, type = "l", xlab = "", ylab = "Global Active Power (kilowatts)"))
## Top right graph
with(hhpc.sub, plot(Voltage, x = time, type = "l", xlab = "datetime", ylab = "Voltage"))
## Bottom left left graph
...
## Bottom right graph
with(hhpc.sub, plot(Global_reactive_power, x = time, type = "l", xlab = "datetime"))
dev.off()
